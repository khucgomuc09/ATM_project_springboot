...
(function(e, d, a, f) {
	...
	var c = function(i, g) {
		...
		var h = (typeof this.$element.prop === "function") ? this.$element
	...
	c.prototype = {
		...
		buildListItems : function(g) {
			...
			if (this.options.prev) {
				var k = g.currentPage > 1 ? g.currentPage - 1
			...
			if (this.options.next) {
				var j = g.currentPage < this.options.totalPages ? g.currentPage + 1
		...
		buildItem : function(i, j) {
			var k = e("<li></li>"), h = e("<a></a>"), g = this.options[i] ? this
					.makeText(this.options[i], j)
					: j;
			k.addClass(this.options[i + "Class"]);
			k.data("page", j);
			k.data("page-type", i);
			k.append(h.attr("href", this.makeHref(j)).addClass(
					this.options.anchorClass).html(g));
			return k
		},
		getPages : function(j) {
			var g = [];
					...
					.each(
							function() {
								...
								case "page":
									...
									break;
								case "first":
									k.toggleClass(i.options.disabledClass,
											g.currentPage === 1);
									break;
								case "last":
									k
		...
		setupEvents : function() {
			var g = this;
			this.$listContainer.off("click").on(
					"click",
					"li",
					function(h) {
						var i = e(this);
						if (i.hasClass(g.options.disabledClass)
		...
		makeText : function(h, g) {
			return h.replace(this.options.pageVariable, g).replace(
					this.options.totalPagesVariable, this.options.totalPages)
		},
		getPageFromQueryString : function(g) {
			var h = this.getSearchString(g), i = new RegExp(
					this.options.pageVariable + "(=([^&#]*)|&|#|$)"), j = i
					.exec(h);
			if (!j || !j[2]) {
				return null
			}
		...
		generateQueryString : function(g, h) {
			var i = this.getSearchString(h), j = new RegExp(
